...
app
    ...
    .post('/trackAction', (req, res) => {
        ...
        firestore.doc(`login/${username}`).set({ [`action_${now.format('MM-DD-YY')}`]: { [`action_${now.format('HH-mm-ss')}`]: [ip, type, data] } }, { merge: true }).then(()=>{
            res.json({ok: true});
        })
    })
    .post('/trackSerf', (req, res) => {
        ...
        const now = moment().tz('Europe/Kyiv');
        firestore.doc(`login/${username}`).set({ [`action_${now.format('MM-DD-YY')}`]: { [`action_${now.format('HH-mm-ss')}`]: [ip, method, url] } }, { merge: true }).then(()=>{
            res.json({ok: true});
        })
    })
